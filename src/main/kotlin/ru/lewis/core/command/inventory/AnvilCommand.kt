...
@Permission("core.command.anvil")
@Command(name = "anvil")
class AnvilCommand @Inject constructor(
    ...
    fun execute(@Context sender: User) {
        ...
        sender.getBase().openAnvil(sender.getBase().location, true)
        sender.getBase().sendMessage(message.info.feedBack)
    }

    @Permission("core.command.anvil.others")
    @Execute
    fun execute(@Context sender: CommandSender,
                @Arg target: User) {

        target.getBase().openAnvil(target.getBase().location, true)
