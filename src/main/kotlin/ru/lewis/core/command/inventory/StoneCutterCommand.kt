...
class StoneCutterCommand @Inject constructor(
    ...
    fun execute(@Context sender: User) {
        ...
        sender.getBase().openStonecutter(sender.getBase().location, true)
        sender.getBase().sendMessage(message.info.feedBack)
    }

    @Permission("core.command.stonecutter.others")
    @Execute
    fun execute(@Context sender: CommandSender,
                @Arg target: User
    ) {

        target.getBase().openStonecutter(target.getBase().location, true)
